// public/app.js — DUPS Photo Contest (round-based voting)
'use strict';
...
(function () {
  ...
  // Persisted short URL — stays the same for the whole meeting
  ...
  function mapJoinError(err) {
    ...
    switch (code) {
      ...
      case 'token_used':    return (err.message || 'This QR code has been replaced. Ask the Administrator for a fresh one.');
  ...
  $('#rotate-qr-btn').addEventListener('click', () => {
    if (confirm('Rotate to a new QR code? Existing voters keep voting, but anyone who has not joined yet must scan the new code.')) {
  ...
  $('#next-round-btn').addEventListener('click', () => {
    if (confirm('Start the next round? Current results will be saved to the archive.')) {
      sendMsg({ type: 'next-round' });
    }
  });

  $('#master-reset-btn').addEventListener('click', () => {
  ...
  // QR fetch — short URL cached for the whole meeting
  let qrLoadInFlight = false;

  async function loadQR() {
    if (cachedShortUrl && $('#qr-image').src && $('#qr-image').src !== window.location.href) {
      $('#qr-url').textContent = cachedShortUrl;
      return;
    }
      ...
      if (!cachedShortUrl) {
        try {
          ...
          if (tiny.ok) {
            ...
            if (short.startsWith('https://tinyurl.com/')) cachedShortUrl = short;
          }
        } catch {}
        if (!cachedShortUrl) cachedShortUrl = data.url;
      }
      $('#qr-url').textContent = cachedShortUrl;
    } catch (e) {
  ...
  function route() {
    if (state.role === 'admin') {
      ...
      if (state.votingOpen && state.photoCount) {
        ...
        const rn = $('#round-number'); if (rn) rn.textContent = state.roundNumber || 1;
        show('screen-admin-voting');
        loadQR();
        return;
      }
      const rn = $('#setup-round-number');
      if (rn) rn.textContent = state.roundNumber > 1 ? `Round ${state.roundNumber}` : '';
  ...
  function renderResults(results) {
    ...
    const rn = $('#results-round-number');
    if (rn) rn.textContent = `Round ${state.roundNumber}`;
    ...
    results.sorted.forEach((row, i) => {
      ...
      const tdRank = document.createElement('td'); tdRank.className = 'col-rank';
      tdRank.textContent = String(i + 1).padStart(2, '0');
      const tdPhoto = document.createElement('td'); tdPhoto.className = 'col-photo';
      tdPhoto.textContent = `Photo No. ${row.photo}`;
      const tdCount = document.createElement('td'); tdCount.className = 'col-votes';
      tdCount.textContent = String(row.count);
      const tdBar = document.createElement('td'); tdBar.className = 'col-bar';
      const bar = document.createElement('span'); bar.className = 'bar';
      const pct = Math.round((row.count / maxCount) * 100);
      bar.style.setProperty('--w', `${pct}%`);
      tdBar.appendChild(bar);
      tr.appendChild(tdRank); tr.appendChild(tdPhoto);
      tr.appendChild(tdCount); tr.appendChild(tdBar);
    ...
    if (results.clusters && results.clusters.length > 0) {
      ...
      ct.textContent = `${results.clusters.length} network${results.clusters.length === 1 ? '' : 's'} produced multiple votes (${total} votes total). This is often legitimate but worth a glance.`;
  ...
  function applyServerState(s) {
    ...
    state.roundNumber      = s.roundNumber || 1;
  ...
  function handleServerMessage(msg) {
    switch (msg.type) {
      ...
      case 'results':
        ...
        break;
      case 'next-round':
        state.roundNumber  = msg.roundNumber || (state.roundNumber + 1);
        state.photoCount   = null;
        state.votingOpen   = false;
        state.votingClosed = false;
        state.myVote       = null;
        state.results      = null;
        const vi = $('#vote-input'); if (vi) vi.value = '';
        const vs = $('#vote-status'); if (vs) { vs.textContent = ''; vs.className = 'status-text'; }
        const pci = $('#photo-count-input'); if (pci) pci.value = '';
        const pcc = $('#photo-count-confirm'); if (pcc) pcc.classList.add('hidden');
        route();
        break;
      case 'qr-rotated':
        cachedShortUrl = null;
